...
export function BirdLivestream() {
  ...
  const streamUrl = import.meta.env.VITE_PI_STREAM_URL || 'http://192.168.2.224:8080/video_feed'
  ...
  useEffect(() => {
    return () => {
      // Clear the image source when component unmounts
      if (imgRef.current) {
        imgRef.current.src = ''
  ...
  // Additional cleanup when stream URL changes
  useEffect(() => {
