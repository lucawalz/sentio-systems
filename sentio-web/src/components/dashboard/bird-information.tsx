...
import { Brain, MapPin, Clock, Eye, Thermometer, Wind, Activity } from "lucide-react"
import { useBirdData } from "../../hooks/useBirdData"
import { useBirdImages } from "../../hooks/useBirdImages"
import { formatDistanceToNow } from "date-fns"

// Helper function to get the most recent bird detection
const getMostRecentBird = (detections: any[]) => {
  if (!detections.length) return null;
  return detections[0]; // Assuming detections are already sorted by most recent
};
...
export function BirdInformation() {
    ...
    return (
        <div className="dashboard-card bg-card/80 backdrop-blur-sm border border-border rounded-3xl p-6 md:p-8 min-h-[480px] relative overflow-hidden shadow-xl">
          <div className="flex items-center justify-center h-full flex-col space-y-4">
            ...
            <div className="text-sm text-muted-foreground">Loading bird analysis...</div>
          </div>
        </div>
    );
  }
    ...
    return (
        <div className="dashboard-card bg-card/80 backdrop-blur-sm border border-border rounded-3xl p-6 md:p-8 min-h-[480px] relative overflow-hidden shadow-xl">
          <div className="flex flex-col items-center justify-center h-full text-center">
            ...
            <div className="text-xl font-semibold mb-2 text-foreground">No Bird Detection Data</div>
            <div className="text-sm text-muted-foreground mb-6 max-w-md">
              The AI bird detection system hasn't identified any birds yet. Please wait for bird activity to be detected.
            </div>
          </div>
        </div>
    );
  ...
  const recentBird = getMostRecentBird(latestDetections);
  const species = recentBird.speciesAiClassified || recentBird.species || "Unknown";
  const confidence = getClassificationConfidence(recentBird);
  const timeAgo = formatDistanceToNow(new Date(recentBird.timestamp), { addSuffix: true });

  // Generate a color for the bird
  const getBirdColor = (species: string): string => {
    const colors = [
  ...
  const birdColor = getBirdColor(species);
        ...
        <div className="relative z-10 h-full flex flex-col">
            ...
            <div className="flex-shrink-0 w-96">
              <div className="relative bg-card/60 backdrop-blur-sm rounded-2xl border border-border/50 p-8">
                ...
                <div className="relative z-10 flex flex-col items-center text-center space-y-6">
                  <div ref={iconRef}>
                    <BirdImage species={species} color={birdColor} />
                  </div>
                  ...
                  <div className="w-full space-y-4">
                    ...
                    <div className="flex items-center justify-center space-x-8">
                      {recentBird.speciesAiClassified && (
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 h-full">
                ...
                <div className="group relative bg-card/60 backdrop-blur-sm rounded-2xl border border-border/50 hover:border-primary/30 transition-all duration-300 overflow-hidden hover:scale-105 p-4">
                  ...
                  <div className="relative z-10">
                    ...
                    <p className="text-sm text-muted-foreground leading-relaxed">
                      ...
                      {confidence > 0.8 ? " active foraging behavior" : " territorial surveillance"}.
                ...
                <div className="group relative bg-card/60 backdrop-blur-sm rounded-2xl border border-border/50 hover:border-primary/30 transition-all duration-300 overflow-hidden hover:scale-105 p-4">
                  ...
                  <div className="relative z-10">
                    <div className="flex items-center space-x-3 mb-3">
                      ...
                      <h5 className="font-semibold text-foreground">Habitat Preferences</h5>
                    </div>
                    <p className="text-sm text-muted-foreground leading-relaxed">
                      {species === "Blue Jay" && "Prefers deciduous and mixed forests, often found near oak trees. Highly intelligent and social, known for complex communication patterns."}
                      {species === "Cardinal" && "Thrives in woodland edges, gardens, and shrublands. Non-migratory species with strong pair bonds and territorial behavior."}
                      {species === "Robin" && "Adaptable to various habitats including lawns, parks, and forest edges. Ground foragers with excellent eyesight for detecting prey."}
                      {species === "Sparrow" && "Urban-adapted species highly successful in human environments. Forms large flocks with complex social hierarchies."}
                      {!["Blue Jay", "Cardinal", "Robin", "Sparrow"].includes(species) &&
                          `This ${species.toLowerCase()} demonstrates remarkable adaptability to local environmental conditions based on detection patterns.`}
                    </p>
                  </div>
                </div>
                ...
                <div className="group relative bg-card/60 backdrop-blur-sm rounded-2xl border border-border/50 hover:border-primary/30 transition-all duration-300 overflow-hidden hover:scale-105 p-4">
                  ...
                  <div className="relative z-10">
                    ...
                    <p className="text-sm text-muted-foreground leading-relaxed">
                      Detection timing correlates with optimal foraging conditions. Current patterns suggest increased
                      activity during {new Date().getHours() < 12 ? "early morning" : "late afternoon"} hours.
                      Environmental indicators show favorable conditions for bird activity.
                    </p>
                  </div>
                </div>
                ...
                <div className="group relative bg-card/60 backdrop-blur-sm rounded-2xl border border-border/50 hover:border-primary/30 transition-all duration-300 overflow-hidden hover:scale-105 p-4">
                  ...
                  <div className="relative z-10">
                    <div className="flex items-center space-x-3 mb-3">
                      ...
                      <h5 className="font-semibold text-foreground">Ecological Impact</h5>
                    </div>
                    <p className="text-sm text-muted-foreground leading-relaxed">
                      This species plays a crucial role in local ecosystem balance through seed dispersal and insect control.
                      Regular detection patterns indicate healthy population dynamics and stable habitat conditions.
                      AI monitoring provides valuable conservation data.
                    </p>
                  </div>
                </div>
              </div>
          ...
          <div className="mt-6 pt-4 border-t border-border/30">
            <div className="text-xs text-muted-foreground opacity-60 text-center">
              AI-powered bird detection • Real-time species identification
            </div>
          </div>
        </div>
      </div>
