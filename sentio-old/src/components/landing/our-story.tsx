...
const milestones = [
  ...
  },
  {
    ...
    description:
      "Developed and deployed the first Raspberry Pi-based biodiversity monitoring station with AI-powered animal identification, contributing valuable data to local wildlife conservation efforts.",
    icon: "🔬",
    color: "#A8D5BA",
  },
  {
        ...
        <div ref={timelineRef} className="relative">
          ...
          {milestones.map((milestone, index) => {
            ...
            return (
              <div
                key={index}
                className={`timeline-item relative flex items-center mb-32 ${isEven ? "flex-row" : "flex-row-reverse"
                  }`}
              >
                <div className={`w-1/2 ${isEven ? "pr-16 text-right" : "pl-16 text-left"}`}>
                  <div
                    className={`timeline-card relative bg-white/5 backdrop-blur-sm border border-white/40 rounded-3xl p-10 shadow-2xl ${milestone.isFuture ? "border-[#A8D5BA]/60 bg-white/10" : ""
                      }`}
                  >
                ...
                <div
                  className={`timeline-icon absolute left-1/2 transform -translate-x-1/2 w-20 h-20 rounded-full border-4 border-black flex items-center justify-center text-2xl shadow-2xl backdrop-blur-sm ${milestone.isFuture ? "ring-4 ring-[#A8D5BA]/30" : ""
                    }`}
