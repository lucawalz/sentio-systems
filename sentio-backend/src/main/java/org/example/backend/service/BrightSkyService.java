...
/**
 * Service responsible for integrating with BrightSky API for weather alerts and radar data.
 ...
 * Radar data is not persisted due to its large size but endpoint information is provided.
 * </p>
 *
 * @author Sentio Team
 * @version 1.0
...
public class BrightSkyService {
     ...
     *
     * @return List of weather alerts for current location, empty list if location cannot be determined
     */
     ...
     * This method integrates with the BrightSky Alerts API to retrieve current alerts,
     * processes the data, and stores it in the database with intelligent update/insert logic.
     *
     ...
     * Includes automatic cleanup and prevents concurrent updates using a thread-safe flag.
     */
    @Transactional
    ...
    public String getRadarEndpointUrl(Float latitude, Float longitude, Integer distance, String format) {
        if (distance == null) distance = 200000;
        if (format == null) format = "compressed";
