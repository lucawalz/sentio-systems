 ...
 * Provides endpoints for fetching historical weather by date ranges and update operations.
...
public class HistoricalWeatherController {
     ...
     * Fetches data for key intervals: 3 days ago, 1 week ago, 1 month ago, 3 months ago, 1 year ago.
     * @return List of historical weather data for current location
     */
    @Operation(summary = "Get historical weather for current location",
            description = "Retrieves historical weather data for the current user's location at key intervals")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully retrieved historical weather data",
                    content = @Content(mediaType = "application/json",
                            array = @ArraySchema(schema = @Schema(implementation = HistoricalWeatherDTO.class))))
    })
    ...
    public ResponseEntity<List<HistoricalWeatherDTO>> getHistoricalWeatherForCurrentLocation() {
        ...
        List<HistoricalWeather> historicalWeather = historicalWeatherService.getHistoricalWeatherForCurrentLocation();
     ...
     */
    @Operation(summary = "Get historical weather for date range",
            description = "Retrieves historical weather data within a specified date range")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully retrieved historical weather for date range",
                    content = @Content(mediaType = "application/json",
                            array = @ArraySchema(schema = @Schema(implementation = HistoricalWeatherDTO.class))))
    })
    @GetMapping("/date-range")
    public ResponseEntity<List<HistoricalWeatherDTO>> getHistoricalWeatherForDateRange(
            @Parameter(description = "Beginning of the date range (inclusive, ISO format: yyyy-MM-dd)")
            @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate startDate,
            @Parameter(description = "End of the date range (inclusive, ISO format: yyyy-MM-dd)")
            @RequestParam @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate endDate) {
        logger.info("Retrieving historical weather for date range: {} to {}", startDate, endDate);
        List<HistoricalWeather> historicalWeather = historicalWeatherService.getHistoricalWeatherForDateRange(startDate, endDate);
        List<HistoricalWeatherDTO> dtos = historicalWeatherMapper.toDTOList(historicalWeather);
     ...
     */
    @Operation(summary = "Get historical weather for specific date",
            description = "Retrieves historical weather data for a specific date")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully retrieved historical weather for date",
                    content = @Content(mediaType = "application/json",
                            schema = @Schema(implementation = HistoricalWeatherDTO.class))),
            @ApiResponse(responseCode = "404", description = "Historical weather not found for date",
                    content = @Content)
    })
    @GetMapping("/date/{date}")
    public ResponseEntity<HistoricalWeatherDTO> getHistoricalWeatherForDate(
            @Parameter(description = "Target date for historical weather (ISO format: yyyy-MM-dd)")
    ...
    @Operation(summary = "Update historical weather data",
            description = "Triggers update of historical weather data for the current user's location")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully updated historical weather data",
                    content = @Content(mediaType = "text/plain")),
            @ApiResponse(responseCode = "500", description = "Failed to update historical weather data",
                    content = @Content(mediaType = "text/plain"))
    })
    @PostMapping("/update")
    public ResponseEntity<String> updateHistoricalWeatherForCurrentLocation() {
        logger.info("Initiating historical weather update for current location");
        try {
            historicalWeatherService.updateHistoricalWeatherForCurrentLocation();
            logger.info("Successfully updated historical weather for current location");
            return ResponseEntity.ok("Historical weather data updated successfully");
        } catch (Exception e) {
            logger.error("Failed to update historical weather for current location", e);
     ...
     */
    @Operation(summary = "Get available cities with historical weather",
            description = "Retrieves a list of cities that have available historical weather data")
    @ApiResponses(value = {
     ...
     */
    @Operation(summary = "Get last update information",
            description = "Retrieves information about the last historical weather update")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully retrieved last update information",
                    content = @Content(mediaType = "application/json"))
    })
    ...
    public ResponseEntity<Map<String, Object>> getLastUpdateInfo() {
        ...
        try {
            // Get the most recent historical weather record (3 days ago, which should be most recent)
            LocalDate threeDaysAgo = LocalDate.now().minusDays(3);
            HistoricalWeather latestHistorical = historicalWeatherService.getHistoricalWeatherForDate(threeDaysAgo);
            ...
            if (latestHistorical != null) {
                ...
                logger.debug("Found latest historical weather from {}", latestHistorical.getUpdatedAt());
            } else {
            ...
            response.put("hasRecentData", latestHistorical != null &&
                    latestHistorical.getUpdatedAt().isAfter(LocalDate.now().minusWeeks(1).atStartOfDay()));
     ...
     * @return Success message
     */
    @Operation(summary = "Cleanup old historical weather",
            description = "Performs cleanup of outdated historical weather data")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully cleaned up old historical weather",
                    content = @Content(mediaType = "text/plain")),
            @ApiResponse(responseCode = "500", description = "Failed to cleanup old historical weather",
                    content = @Content(mediaType = "text/plain"))
    })
    ...
    /**
     * Retrieves key historical dates comparison data.
     * Returns weather for 3 days ago, 1 week ago, 1 month ago, 3 months ago, and 1 year ago.
     * @return Map with labeled historical weather data
     */
    @Operation(summary = "Get historical weather comparison",
            description = "Retrieves key historical dates comparison data for analysis and comparison")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully retrieved historical comparison data",
                    content = @Content(mediaType = "application/json"))
    })
    @GetMapping("/comparison")
    public ResponseEntity<Map<String, Object>> getHistoricalComparison() {
        logger.info("Retrieving historical weather comparison data");
        ...
        comparisonDates.put("oneWeekAgo", now.minusWeeks(1));
        ...
        for (Map.Entry<String, LocalDate> entry : comparisonDates.entrySet()) {
            HistoricalWeather historical = historicalWeatherService.getHistoricalWeatherForDate(entry.getValue());
            if (historical != null) {
        ...
        logger.debug("Retrieved historical comparison data for {} periods", comparison.size());
