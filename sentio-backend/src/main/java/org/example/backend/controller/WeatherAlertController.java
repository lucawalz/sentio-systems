 ...
 * Provides endpoints for fetching alerts by location, severity, and managing alert updates.
...
public class WeatherAlertController {
    ...
    /**
     * Retrieves weather alerts for the current user's location based on IP geolocation.
     * @param lang Language preference (de for German, en for English, default: en)
     * @return List of weather alerts for current location
     */
    @Operation(summary = "Get alerts for current location",
            description = "Retrieves weather alerts for the current user's location based on IP geolocation")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully retrieved alerts for current location",
                    content = @Content(mediaType = "application/json",
                            array = @ArraySchema(schema = @Schema(implementation = WeatherAlertDTO.class))))
    })
    @GetMapping("/current-location")
    public ResponseEntity<List<WeatherAlertDTO>> getCurrentLocationAlerts(
            @Parameter(description = "Language preference (de for German, en for English)")
    ...
    @Operation(summary = "Get active weather alerts",
            description = "Retrieves all currently active weather alerts (not expired)")
    @ApiResponses(value = {
     ...
     */
    @Operation(summary = "Get alerts by warn cell ID",
            description = "Retrieves weather alerts by municipality warn cell ID")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully retrieved alerts for warn cell",
                    content = @Content(mediaType = "application/json",
                            array = @ArraySchema(schema = @Schema(implementation = WeatherAlertDTO.class))))
    })
    @GetMapping("/warn-cell/{warnCellId}")
    public ResponseEntity<List<WeatherAlertDTO>> getAlertsByWarnCellId(
            @Parameter(description = "Municipality warn cell ID")
            @PathVariable Long warnCellId,
            @Parameter(description = "Language preference (de for German, en for English)")
            @RequestParam(defaultValue = "en") String lang) {
     ...
     */
    @Operation(summary = "Get alerts by city",
            description = "Retrieves weather alerts by city name")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully retrieved alerts for city",
                    content = @Content(mediaType = "application/json",
                            array = @ArraySchema(schema = @Schema(implementation = WeatherAlertDTO.class))))
    })
    ...
    @Operation(summary = "Get alerts by severity",
            description = "Retrieves weather alerts by severity level")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully retrieved alerts for severity level",
                    content = @Content(mediaType = "application/json",
                            array = @ArraySchema(schema = @Schema(implementation = WeatherAlertDTO.class))))
    })
    @GetMapping("/severity/{severity}")
    public ResponseEntity<List<WeatherAlertDTO>> getAlertsBySeverity(
            @Parameter(description = "Severity level (minor, moderate, severe, extreme)")
     ...
     */
    @Operation(summary = "Get active alerts for location",
            description = "Retrieves currently active alerts for a specific location")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully retrieved active alerts for location",
                    content = @Content(mediaType = "application/json",
                            array = @ArraySchema(schema = @Schema(implementation = WeatherAlertDTO.class))))
    })
    @GetMapping("/location")
    public ResponseEntity<List<WeatherAlertDTO>> getActiveAlertsForLocation(
            ...
            @Parameter(description = "Warn cell ID (optional)")
            @RequestParam(required = false) Long warnCellId,
            @Parameter(description = "Language preference (de for German, en for English)")
            @RequestParam(defaultValue = "en") String lang) {
     ...
     */
    @Operation(summary = "Get recent weather alerts",
            description = "Retrieves recent weather alerts (last 24 hours)")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully retrieved recent alerts",
                    content = @Content(mediaType = "application/json",
                            array = @ArraySchema(schema = @Schema(implementation = WeatherAlertDTO.class))))
    })
    @GetMapping("/recent")
     ...
     */
    @Operation(summary = "Update weather alerts",
            description = "Triggers update of weather alerts for the current user's location")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully updated weather alerts",
                    content = @Content(mediaType = "text/plain")),
            @ApiResponse(responseCode = "500", description = "Failed to update weather alerts",
                    content = @Content(mediaType = "text/plain"))
    })
    @PostMapping("/update")
     ...
     * @return Success message
     */
    @Operation(summary = "Cleanup expired alerts",
            description = "Performs cleanup of expired weather alerts")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully cleaned up expired alerts",
                    content = @Content(mediaType = "text/plain")),
            @ApiResponse(responseCode = "500", description = "Failed to cleanup expired alerts",
                    content = @Content(mediaType = "text/plain"))
    })
     ...
     */
    @Operation(summary = "Get radar endpoint configuration",
            description = "Retrieves radar data endpoint configuration for the current location")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully generated radar endpoint configuration",
                    content = @Content(mediaType = "application/json"))
    })
    @GetMapping("/radar/endpoint")
    public ResponseEntity<Map<String, Object>> getRadarEndpoint(
            @Parameter(description = "Distance in meters to each side of location (default: 200000)")
            @RequestParam(required = false) Integer distance,
            @Parameter(description = "Data format (compressed, bytes, plain - default: compressed)")
            @RequestParam(defaultValue = "compressed") String format) {
     ...
     */
    @Operation(summary = "Get cities with active alerts",
            description = "Retrieves a list of cities that currently have active weather alerts")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully retrieved cities with active alerts",
                    content = @Content(mediaType = "application/json",
