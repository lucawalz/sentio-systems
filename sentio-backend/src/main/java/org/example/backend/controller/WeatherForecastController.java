 ...
 * Provides endpoints for fetching forecasts by location, date ranges, and update operations.
...
public class WeatherForecastController {
     ...
     * Fetches data for key intervals: 3 days ago, 2 weeks ago, 1 month ago, 3 months ago, 1 year ago.
     * @return List of historical weather data for current location
     */
    @Operation(summary = "Get forecast for current location",
            description = "Retrieves weather forecast data for the current user's location")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully retrieved forecast for current location",
                    content = @Content(mediaType = "application/json",
                            array = @ArraySchema(schema = @Schema(implementation = WeatherForecastDTO.class))))
    })
    @GetMapping("/current-location")
    public ResponseEntity<List<WeatherForecastDTO>> getCurrentLocationForecast() {
        logger.info("Retrieving forecast for current location");
        ...
        logger.debug("Retrieved {} forecasts for current location", dtos.size());
     ...
     */
    @Operation(summary = "Get upcoming forecasts",
            description = "Retrieves all upcoming weather forecasts starting from today")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully retrieved upcoming forecasts",
                    content = @Content(mediaType = "application/json",
                            array = @ArraySchema(schema = @Schema(implementation = WeatherForecastDTO.class))))
    })
    @GetMapping("/upcoming")
     ...
     */
    @Operation(summary = "Get forecasts for date range",
            description = "Retrieves weather forecasts within a specified date range")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully retrieved forecasts for date range",
                    content = @Content(mediaType = "application/json",
                            array = @ArraySchema(schema = @Schema(implementation = WeatherForecastDTO.class))))
    })
     ...
     */
    @Operation(summary = "Get forecasts for specific date",
            description = "Retrieves all weather forecasts for a specific date")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully retrieved forecasts for date",
                    content = @Content(mediaType = "application/json",
                            array = @ArraySchema(schema = @Schema(implementation = WeatherForecastDTO.class))))
    })
    @GetMapping("/date/{date}")
    public ResponseEntity<List<WeatherForecastDTO>> getForecastsForDate(
            @Parameter(description = "Target date for forecasts (ISO format: yyyy-MM-dd)")
     ...
     */
    @Operation(summary = "Get latest forecast for specific date",
            description = "Retrieves the most recent forecast for a specific date")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully retrieved latest forecast for date",
                    content = @Content(mediaType = "application/json",
                            schema = @Schema(implementation = WeatherForecastDTO.class))),
            @ApiResponse(responseCode = "404", description = "No forecast found for date",
                    content = @Content)
    })
    @GetMapping("/latest/{date}")
    public ResponseEntity<WeatherForecastDTO> getLatestForecastForDate(
            @Parameter(description = "Target date for forecast (ISO format: yyyy-MM-dd)")
            @PathVariable @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate date) {
     ...
     */
    @Operation(summary = "Get recent forecasts",
            description = "Retrieves weather forecasts created within the specified number of hours")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully retrieved recent forecasts",
                    content = @Content(mediaType = "application/json",
                            array = @ArraySchema(schema = @Schema(implementation = WeatherForecastDTO.class))))
    })
    @GetMapping("/recent")
    public ResponseEntity<List<WeatherForecastDTO>> getRecentForecasts(
            @Parameter(description = "Number of hours to look back from current time (default: 24)")
     ...
     */
    @Operation(summary = "Update weather forecasts",
            description = "Triggers update of weather forecasts for the current user's location")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully updated weather forecasts",
                    content = @Content(mediaType = "text/plain")),
            @ApiResponse(responseCode = "500", description = "Failed to update weather forecasts",
                    content = @Content(mediaType = "text/plain"))
    })
    @PostMapping("/update")
     ...
     */
    @Operation(summary = "Get available cities with forecasts",
            description = "Retrieves a list of cities that have available weather forecasts")
    @ApiResponses(value = {
    ...
    /**
     * Retrieves information about the last forecast update and next expected update.
     * @return Map containing last update timestamps and next update estimate
     */
    @Operation(summary = "Get last update information",
            description = "Retrieves information about the last forecast update and next expected update")
    @ApiResponses(value = {
     ...
     * @return Success message
     */
    @Operation(summary = "Cleanup old forecasts",
            description = "Performs cleanup of outdated weather forecast data")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Successfully cleaned up old forecasts",
                    content = @Content(mediaType = "text/plain")),
            @ApiResponse(responseCode = "500", description = "Failed to cleanup old forecasts",
                    content = @Content(mediaType = "text/plain"))
    })
