...
@WebMvcTest(
        controllers = AnimalDetectionController.class,
...
class AnimalDetectionControllerTest {
    ...
    @Autowired AnimalDetectionService animalDetectionService;
    @Autowired AnimalDetectionMapper mapper;

    @TestConfiguration
    static class TestBeans {
        @Bean AnimalDetectionService animalDetectionService() { return mock(AnimalDetectionService.class); }
        @Bean AnimalDetectionMapper animalDetectionMapper() { return mock(AnimalDetectionMapper.class); }
    }
    ...
    @BeforeEach
    void resetMocks() {
        // wichtig, damit "verifyNoMoreInteractions" nicht von anderen Tests beeinflusst wird
        reset(animalDetectionService, mapper);
    ...
    void getLatestDetections_returns200_andUsesDefaultLimit() throws Exception {
        when(animalDetectionService.getLatestDetections(10)).thenReturn(List.of(e1, e2));
        ...
        verify(animalDetectionService).getLatestDetections(10);
        verify(mapper).toDTOList(List.of(e1, e2));
        verifyNoMoreInteractions(animalDetectionService, mapper);
    }

    @Test
    void getLatestDetections_respectsLimitParam() throws Exception {
        when(animalDetectionService.getLatestDetections(3)).thenReturn(List.of(e1));
        ...
        verify(animalDetectionService).getLatestDetections(3);
    ...
    void getRecentDetections_returns200_andUsesDefaultHours() throws Exception {
        when(animalDetectionService.getRecentDetections(24)).thenReturn(List.of(e1));
        when(mapper.toDTOList(List.of(e1))).thenReturn(List.of(d1));
        ...
        verify(animalDetectionService).getRecentDetections(24);
        verify(mapper).toDTOList(List.of(e1));
        verifyNoMoreInteractions(animalDetectionService, mapper);
    }
    ...
    void getDetectionsByDate_parsesIsoDate_andReturns200() throws Exception {
        ...
        when(animalDetectionService.getDetectionsByDate(date)).thenReturn(List.of(e1, e2));
        when(mapper.toDTOList(List.of(e1, e2))).thenReturn(List.of(d1, d2));
        ...
        verify(animalDetectionService).getDetectionsByDate(date);
        verify(mapper).toDTOList(List.of(e1, e2));
        verifyNoMoreInteractions(animalDetectionService, mapper);
    }
    ...
    void getDetectionsBySpecies_passesPageableCorrectly() throws Exception {
        when(animalDetectionService.getDetectionsBySpecies(eq("sparrow"), any(Pageable.class)))
        ...
        verify(animalDetectionService).getDetectionsBySpecies(eq("sparrow"), pageableCaptor.capture());
    ...
    void getDetectionsByAnimalType_passesPageableCorrectly() throws Exception {
        when(animalDetectionService.getDetectionsByAnimalType(eq("bird"), any(Pageable.class)))
                .thenReturn(List.of(e1));
        when(mapper.toDTOList(List.of(e1))).thenReturn(List.of(d1));
        ...
        var pageableCaptor = ArgumentCaptor.forClass(Pageable.class);
        verify(animalDetectionService).getDetectionsByAnimalType(eq("bird"), pageableCaptor.capture());
    ...
    void getDetectionsByDevice_passesPageableCorrectly() throws Exception {
        when(animalDetectionService.getDetectionsByDevice(eq("dev-1"), any(Pageable.class)))
        ...
        var pageableCaptor = ArgumentCaptor.forClass(Pageable.class);
        verify(animalDetectionService).getDetectionsByDevice(eq("dev-1"), pageableCaptor.capture());
    ...
    void getDetectionSummary_returns200() throws Exception {
        ...
        when(animalDetectionService.getDetectionSummary(24)).thenReturn(summary);

        mockMvc.perform(get("/api/animals/summary"))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                // falls AnimalDetectionSummary ein Feld "totalDetections" hat (wie im Controller geloggt)
                .andExpect(jsonPath("$.totalDetections").value(42));

        verify(animalDetectionService).getDetectionSummary(24);
        verifyNoMoreInteractions(animalDetectionService, mapper);
    }

    @Test
    void getSpeciesCount_returns200() throws Exception {
        when(animalDetectionService.getSpeciesCount(24)).thenReturn(Map.of("Sparrow", 3L, "Fox", 1L));
        ...
        verify(animalDetectionService).getSpeciesCount(24);
        verifyNoMoreInteractions(animalDetectionService, mapper);
    }

    @Test
    void getAnimalTypeCount_returns200() throws Exception {
        when(animalDetectionService.getAnimalTypeCount(24)).thenReturn(Map.of("bird", 5L, "mammal", 2L));
        ...
        verify(animalDetectionService).getAnimalTypeCount(24);
        verifyNoMoreInteractions(animalDetectionService, mapper);
    }
    ...
    void getHourlyActivity_parsesIsoDate_andReturns200() throws Exception {
        LocalDate date = LocalDate.of(2025, 12, 18);
        when(animalDetectionService.getHourlyActivity(date)).thenReturn(Map.of(8, 2L, 9, 5L));
        ...
        verify(animalDetectionService).getHourlyActivity(date);
        verifyNoMoreInteractions(animalDetectionService, mapper);
    }

    @Test
    void getHighConfidenceDetections_usesDefaults() throws Exception {
        when(animalDetectionService.getHighConfidenceDetections(0.8f, 24)).thenReturn(List.of(e1));
        when(mapper.toDTOList(List.of(e1))).thenReturn(List.of(d1));
        ...
        verify(animalDetectionService).getHighConfidenceDetections(0.8f, 24);
        verify(mapper).toDTOList(List.of(e1));
        verifyNoMoreInteractions(animalDetectionService, mapper);
    }
    ...
    void getHighConfidenceDetections_respectsParams() throws Exception {
        when(animalDetectionService.getHighConfidenceDetections(0.9f, 6)).thenReturn(List.of(e2));
        when(mapper.toDTOList(List.of(e2))).thenReturn(List.of(d2));
        ...
        verify(animalDetectionService).getHighConfidenceDetections(0.9f, 6);
        verify(mapper).toDTOList(List.of(e2));
        verifyNoMoreInteractions(animalDetectionService, mapper);
    }
    ...
    void getDetectionById_returns200_whenFound() throws Exception {
        when(animalDetectionService.getDetectionById(1L)).thenReturn(Optional.of(e1));
        ...
        verify(animalDetectionService).getDetectionById(1L);
    ...
    void getDetectionById_returns404_whenMissing() throws Exception {
        when(animalDetectionService.getDetectionById(999L)).thenReturn(Optional.empty());

        mockMvc.perform(get("/api/animals/999"))
                .andExpect(status().isNotFound());

        verify(animalDetectionService).getDetectionById(999L);
        verifyNoMoreInteractions(animalDetectionService, mapper);
    }
    ...
    void recordDetection_returns200_andCallsMapperAndService() throws Exception {
        ...
        when(animalDetectionService.saveAnimalDetection(e1)).thenReturn(e1);
        when(mapper.toDTO(e1)).thenReturn(d1);
        ...
        verify(animalDetectionService).saveAnimalDetection(e1);
        verify(mapper).toDTO(e1);
        verifyNoMoreInteractions(animalDetectionService, mapper);
    }
    ...
    void deleteDetection_returns204_whenDeleted() throws Exception {
        when(animalDetectionService.deleteDetection(1L)).thenReturn(true);

        mockMvc.perform(delete("/api/animals/1"))
                .andExpect(status().isNoContent());

        verify(animalDetectionService).deleteDetection(1L);
        verifyNoMoreInteractions(animalDetectionService, mapper);
    }

    @Test
    void deleteDetection_returns404_whenNotFound() throws Exception {
        when(animalDetectionService.deleteDetection(999L)).thenReturn(false);

        mockMvc.perform(delete("/api/animals/999"))
                .andExpect(status().isNotFound());

        verify(animalDetectionService).deleteDetection(999L);
        verifyNoMoreInteractions(animalDetectionService, mapper);
    }

    @Test
    void getSystemStats_returns200() throws Exception {
        when(animalDetectionService.getSystemStats()).thenReturn(Map.of("totalDetections", 10));
        ...
        verify(animalDetectionService).getSystemStats();
        verifyNoMoreInteractions(animalDetectionService, mapper);
    }

    @Test
    void getAllSpecies_returns200() throws Exception {
        when(animalDetectionService.getAllSpecies()).thenReturn(List.of("Sparrow", "Fox"));
        ...
        verify(animalDetectionService).getAllSpecies();
        verifyNoMoreInteractions(animalDetectionService, mapper);
    }

    @Test
    void getAllAnimalTypes_returns200() throws Exception {
        when(animalDetectionService.getAllAnimalTypes()).thenReturn(List.of("bird", "mammal"));
        ...
        verify(animalDetectionService).getAllAnimalTypes();
        verifyNoMoreInteractions(animalDetectionService, mapper);
    }

    @Test
    void getMammalDetections_returns200() throws Exception {
        when(animalDetectionService.getMammalDetections(24)).thenReturn(List.of(e2));
        when(mapper.toDTOList(List.of(e2))).thenReturn(List.of(d2));
        ...
        verify(animalDetectionService).getMammalDetections(24);
        verify(mapper).toDTOList(List.of(e2));
        verifyNoMoreInteractions(animalDetectionService, mapper);
    }
}