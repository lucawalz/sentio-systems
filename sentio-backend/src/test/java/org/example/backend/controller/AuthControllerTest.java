...
class AuthControllerTest {
    ...
    @Autowired CookieAuthService cookieAuthService;

    @TestConfiguration
    static class TestBeans {
        @Bean AuthService authService() { return mock(AuthService.class); }
        @Bean CookieAuthService cookieAuthService() { return mock(CookieAuthService.class); }
    }
    @AfterEach
    void resetMocks() {
    ...
    void login_setsCookies_andReturns200() throws Exception {
        ...
        mockMvc.perform(post("/api/auth/login")
                ...
                .andExpect(header().stringValues(HttpHeaders.SET_COOKIE, access.toString(), refresh.toString()));
    ...
    void refresh_whenServiceThrows_clearsCookies_andReturns401() throws Exception {
        ...
        mockMvc.perform(post("/api/auth/refresh")
                ...
                .andExpect(header().stringValues(HttpHeaders.SET_COOKIE,
                        hasItem(allOf(containsString("access_token="), containsString("Max-Age=0"), containsString("Path=/")))))
                .andExpect(header().stringValues(HttpHeaders.SET_COOKIE,
                        hasItem(allOf(containsString("refresh_token="), containsString("Max-Age=0"), containsString("Path=/")))));
