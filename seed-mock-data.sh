...
if mqtt_publish_with_token "weather/status" "$STATUS_MSG" "$DEVICE1_ID" "$DEVICE1_TOKEN"; then
...
if mqtt_publish_with_token "animal_detection/status" "$STATUS_MSG" "$DEVICE2_ID" "$DEVICE2_TOKEN"; then
...
while true; do
    ...
    mqtt_publish_with_token "weather/status" "$STATUS_MSG1" "$DEVICE1_ID" "$DEVICE1_TOKEN" 2>/dev/null
    ...
    mqtt_publish_with_token "animal_detection/status" "$STATUS_MSG2" "$DEVICE2_ID" "$DEVICE2_TOKEN" 2>/dev/null
